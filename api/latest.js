...
function mapManga(manga) {
  ...
  return {
    ...
    latestUploadedAt: attrs.latestUploadedChapter,
...
module.exports = async (req, res) => {
  ...
  const { limit = 24, since } = req.query;

  try {
    const params = new URLSearchParams();
    params.append('limit', String(Math.min(parseInt(limit), 100)));
    ...
    if (since) {
      params.append('updatedAtSince', since);
    }

    const { status, body } = await httpsGet(`https://api.mangadex.org/manga?${params.toString()}`);
    if (status !== 200 || !body?.data) return res.status(200).json({ results: [], total: 0, serverTime: new Date().toISOString() });

    return res.status(200).json({
      results: body.data.map(mapManga),
      total: body.total,
