...
export default function DetailPage() {
  ...
  function handleBookmark() {
    if (!manga) return;
    const result = toggleBookmark({ ...manga, hid });
  ...
  if (error || !manga) {
    return (
      <div className="max-w-6xl mx-auto px-4 py-24 text-center">
        ...
        <Link to="/" className="text-crimson-400 border border-crimson-600/30 rounded-full px-4 py-2 text-sm transition-colors">Back to Home</Link>
      </div>
    );
  }
    ...
    <main className="max-w-6xl mx-auto px-4 py-8 animate-slide-up">
      ...
      <div className="flex flex-col md:flex-row gap-8 mb-10">
        <div className="shrink-0 w-44 md:w-52 mx-auto md:mx-0">
          <div className="aspect-[2/3] rounded-xl overflow-hidden border border-ink-600 bg-ink-700">
            {manga.poster
          ...
          <div className="mt-3 flex flex-col gap-2">
            {progress && (
              <button onClick={() => navigate(`/read/${progress.chapterId}`)}
            ...
            {chapters.length > 0 && !progress && (
              <button onClick={() => navigate(`/read/${chapters[chapters.length - 1].id}`)}
                className="w-full text-center bg-crimson-600 hover:bg-crimson-500 text-white text-sm font-medium py-2.5 rounded-lg transition-colors">
                Start Reading
              </button>
            )}
                ...
                return (
                  <button key={ch.id} onClick={() => navigate(`/read/${ch.id}?manga=${hid}&ch=${ch.chapter}`)}
                    ...
                    <div className="flex items-center gap-3 min-w-0">
                      {isRead && <div className="w-1.5 h-1.5 rounded-full bg-ash-500 shrink-0" />}
                      {isCurrent && <div className="w-1.5 h-1.5 rounded-full bg-crimson-400 shrink-0" />}
                      {!isRead && !isCurrent && <div className="w-1.5 h-1.5 shrink-0" />}
                      <span className={`font-mono text-sm shrink-0 ${isCurrent ? 'text-crimson-400' : isRead ? 'text-ash-500' : 'text-crimson-400'}`}>
                        Ch.{ch.chapter || '?'}
                      </span>
                      <span className={`text-sm truncate transition-colors ${isRead ? 'text-ash-500' : 'text-ash-300 group-hover:text-ash-100'}`}>
